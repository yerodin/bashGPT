...
class CommandNotFound(object):
    ...
    def run_with_gpt(self, cmd_args):
        prompt = _(f"run {cmd_args[0]} with chatGPT? (N/y)")
        if sys.version >= '3':
            answer = input(prompt)
        ...
        if answer.lower() == _("y"):
            print(_(f"executing {' '.join(cmd_args)} with chatGPT assistance..."), file=self.output_fd)
            sp_args = ["python3", "/usr/share/command-not-found-bashGPT/lookup.py"]
            sp_args.extend(cmd_args)
            subprocess.call(sp_args, shell=False)
        print()
    ...
    def install_prompt(self, package_name, cmd_args):
        ...
        if package_name:
            ...
            if answer.lower() == _("y"):
                ...
                self.run_with_gpt(cmd_args)

    def print_spelling_suggestions(self, word, cmd_args, mispell_packages, mispell_snaps, max_alt=15):
    ...
    def advice_single_snap_package(self, command, cmd_args, packages, snaps):
    ...
    def advice_single_deb_package(self, command, cmd_args, packages, snaps):
        print("??????"+command)
        self._print_exact_header(command)
        if self.euid == 0:
    ...
    def advice_multi_deb_package(self, command, cmd_args, packages, snaps):
        self._print_exact_header(command)
    ...
    def advice_multi_snap_packages(self, command, cmd_args, packages, snaps):
        self._print_exact_header(command)
        pad = max([len(s[0]) for s in snaps+packages])
    ...
    def advice_multi_mixed_packages(self,cmd_args, command, packages, snaps):
        self._print_exact_header(command)
        pad = max([len(s[0]) for s in snaps+packages])
        for i, snap in enumerate(snaps):
            ver=""
    ...
    def advise(self, command,cmd_args, ignore_installed=False):
        ...
        if len(packages) == 0 and len(snaps) == 0:
            ...
            if len(mispell_packages) > 0 or len(mispell_snaps) > 0:
                self.print_spelling_suggestions(command,cmd_args, mispell_packages, mispell_snaps)
        elif len(packages) == 0 and len(snaps) == 1:
            self.advice_single_snap_package(command, cmd_args, packages, snaps)
        elif len(snaps) > 0 and len(packages) == 0:
            self.advice_multi_snap_packages(command, cmd_args, packages, snaps)
        elif len(packages) == 1 and len(snaps) == 0:
            self.advice_single_deb_package(command, cmd_args, packages, snaps)
        elif len(packages) > 1 and len(snaps) == 0:
            self.advice_multi_deb_package(command, cmd_args, packages, snaps)
        elif len(packages) > 0 and len(snaps) > 0:
            self.advice_multi_mixed_packages(command, cmd_args, packages, snaps)
